...
Nhóm tùy chọn có sẵn default, thường không cần đụng: chính sách giữ chỗ, job dọn hold, storage local, MoMo/VNPay payment mock ports. Riêng QR ký vé bỏ trống sẽ dùng khóa dev; bản production bắt buộc phải tự sinh và điền.
...
Đây là các giá trị mặc định, không cần lấy từ đâu, chỉ tự đặt cho khớp môi trường. Cổng mặc định: API 3000, Web 3001, MoMo payment mock 4101, VNPay payment mock 4102. Nếu đổi `PORT` của API thì phải cập nhật `VITE_API_BASE_URL` và tất cả URL return/IPN của payment cho khớp.
...
Hai cổng của server giả lập thanh toán trong `apps/payment-mocks`, cho phép demo MoMo và VNPay mà không cần tài khoản merchant thật. Chạy bằng hai script riêng: `npm run dev:payment:momo` cho MoMo mock và `npm run dev:payment:vnpay` cho VNPay mock.
