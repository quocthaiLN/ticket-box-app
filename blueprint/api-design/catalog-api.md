...
- `ticket_types`: `id`, `concert_id`, `seat_zone_id`, `name`, `description`, `price`, `currency`, `total_quantity`, `available_quantity`, `held_quantity`, `sold_quantity`, `max_per_user`, `sale_start_at`, `sale_end_at`, `status`.

Public API không trả các field vận hành nhạy cảm nếu không cần, ví dụ `organizer_id`, `held_quantity`, audit hoặc internal counters.
...
inventory:concert:{concert_id}
  ticket_type:{ticket_type_id}:available_quantity
...
- `available_quantity = total_quantity`, `held_quantity = 0`, `sold_quantity = 0` khi tạo mới.
- `max_per_user > 0`.
- `sale_end_at > sale_start_at`.
- Ghi `ticket_inventory_events` loại `ADMIN_ADJUST` nếu cần audit tồn kho ban đầu.
...
- Không sửa trực tiếp `available_quantity`, `held_quantity`, `sold_quantity` bằng endpoint này.
- Tăng/giảm tổng tồn kho sau khi đã bán vé phải đi qua nghiệp vụ inventory adjustment riêng của Ticketing/Admin Inventory.
