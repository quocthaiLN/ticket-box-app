...
7. Khi payment thành công, giảm `held_quantity`, tăng `paid_quantity`, chuyển order sang `CONFIRMED` và phát hành ticket.
...
- Counter lệch do lỗi hệ thống: chạy job reconcile từ `tickets`, `orders` và `order_items` để sửa counter.
...
- Chỉ tính các trạng thái còn hiệu lực: order `HELD`, order `CONFIRMED`, ticket `ISSUED` hoặc `CHECKED_IN` tùy policy quota sau check-in.
