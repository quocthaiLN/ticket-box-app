...
*   **Web App (React/Express.js):** Ứng dụng Single Page hoặc Server-Side Rendering. Phục vụ khán giả mua vé và Ban tổ chức quản trị.
*   **Mobile App (React Native):** Ứng dụng dành riêng cho nhân viên soát vé. Có tích hợp **SQLite/Room DB** cục bộ để tải trước danh sách vé hợp lệ, cho phép hoạt động và ghi nhận check-in ngay cả khi sân vận động mất sóng.
...
*   **Message Broker (BullMQ):** Phân tải hệ thống. Thay vì bắt Ticketing Module phải chờ gọi API gửi Email hoặc lưu Log, nó chỉ ném một "Sự kiện" (Event) vào Broker rồi trả kết quả ngay cho người dùng.
...
sequenceDiagram
    ...
    participant TS as Ticketing Module
    participant DB as PostgreSQL (Primary DB)
    participant RD as Redis Cluster
    participant PM as Payment Module
    participant VNP as Cổng VNPAY/MoMo

    User->>GW: Gửi yêu cầu đặt vé
    Note over User,GW: Idempotency-Key, TicketType, Qty

    GW->>TS: Chuyển tiếp request sau khi qua Rate Limiting

    rect rgb(230, 245, 255)
        Note right of TS: Giai đoạn 1 - Short-lived Transaction

        TS->>DB: BEGIN Transaction
        TS->>DB: SELECT ... FOR UPDATE
        DB-->>TS: Trả về số lượng vé hiện tại
        ...
        alt Số lượng vé đủ
            ...
            TS-->>User: HTTP 400 - Vé đã hết
        end
    end

    rect rgb(240, 240, 240)
        Note right of TS: Giai đoạn 2 - Khởi tạo giữ chỗ tạm thời

        TS->>RD: SET order hold TTL 10 phút

        TS->>PM: Tạo phiên thanh toán
        PM->>VNP: Gọi API tạo payment URL
        VNP-->>PM: Trả về payment URL

        PM-->>TS: Trả URL thanh toán
        TS-->>User: Redirect sang VNPAY/MoMo
    end

    Note over User,VNP: Người dùng thanh toán trong vòng 10 phút
    ...
    alt Thanh toán thành công
        ...
        VNP->>PM: IPN thất bại hoặc timeout
        PM->>TS: Event Order Expired Or Failed
...
| --- | --- | --- |
| **Frontend (Web)** | **React (App Router)** | Phát triển trang chủ, chi tiết Concert và Admin Dashboard. Hỗ trợ SSR (Server-Side Rendering) tốt cho SEO và chịu tải. |
...
| **Backend Framework** | **Express.js (Node.js)** | Xây dựng các Module nghiệp vụ. Phù hợp cực tốt cho các luồng I/O-intensive bất đồng bộ. Express.js có Module system built-in, decorator-based, rất phù hợp để implement Modular Monolith một cách có cấu trúc.|
