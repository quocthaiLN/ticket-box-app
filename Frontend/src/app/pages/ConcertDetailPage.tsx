...
import { Calendar, MapPin, Clock, ChevronLeft, Info, AlertTriangle, CheckCircle, HelpCircle, Music, LogIn } from "lucide-react";
    ...
    <div style={{ background: "#08080E", minHeight: "100vh", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {showLoginPrompt && (
        ...
        >
          ...
          >
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(232,49,91,0.12)", border: "1px solid rgba(232,49,91,0.25)" }}>
              <LogIn className="w-8 h-8" style={{ color: "#E8315B" }} />
            </div>
            <h3 className="text-lg font-semibold mb-2" style={{ color: "#F0EDEB", fontFamily: "'Cormorant Garamond', serif" }}>
              Đăng nhập để mua vé
            </h3>
            <p className="text-sm mb-6" style={{ color: "#8585A0" }}>
              Bạn cần đăng nhập để tiếp tục mua vé và chọn chỗ ngồi.
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={() => { sessionStorage.setItem("redirectAfterLogin", `/concerts/${concert.slug}`); navigate("/login"); }}
              ...
              <button
                onClick={() => { sessionStorage.setItem("redirectAfterLogin", `/concerts/${concert.slug}`); navigate("/register"); }}
                className="w-full py-3 rounded-xl font-medium text-sm transition-colors hover:bg-white/10"
                style={{ background: "rgba(255,255,255,0.06)", color: "#F0EDEB", border: "1px solid rgba(255,255,255,0.1)" }}
              >
                Tạo tài khoản miễn phí
              </button>
              <button onClick={() => setShowLoginPrompt(false)} className="text-sm" style={{ color: "#8585A0" }}>
                Tiếp tục xem
              </button>
            </div>
          </div>
