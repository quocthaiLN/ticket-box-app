...
export async function getInventoryByTicketTypeId(ticketTypeId: string) {
  ...
  if (!row) {
    throw new InventoryError('TICKET_TYPE_NOT_FOUND', 'Ticket type not found', 404);
  }
...
export async function holdInventory(req: HoldRequest, idempotencyKey: string) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (ticketTypes.length !== ticketTypeIds.length) {
      throw new InventoryError('TICKET_TYPE_NOT_FOUND', 'One or more ticket types not found', 404);
    }
    ...
    for (const item of sortedItems) {
      ...
      if (available < item.quantity) {
        throw new InventoryError('TICKET_SOLD_OUT', `Not enough available tickets for type ${item.ticket_type_id}`, 409);
      }
    }
    ...
    const itemResults: Array<{ ticket_type_id: string; quantity: number; available_quantity_after: number }> = [];
    ...
    for (const item of sortedItems) {
      ...
      const available = tt.totalQuantity - tt.heldQuantity - tt.soldQuantity - item.quantity;
      itemResults.push({ ticket_type_id: item.ticket_type_id, quantity: item.quantity, available_quantity_after: available });
    }
...
export async function releaseInventory(req: ReleaseRequest) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const newStatus = req.reason === 'HOLD_EXPIRED' ? OrderStatus.EXPIRED : OrderStatus.CANCELLED;
    const now = new Date();
    ...
    await tx.order.update({
      ...
      data: {
        status: newStatus,
        ...(newStatus === OrderStatus.EXPIRED ? { expiredAt: now } : { cancelledAt: now, cancelledReason: req.reason }),
      },
    });
...
export async function adjustInventory(ticketTypeId: string, req: InventoryAdjustmentRequest, actorUserId?: string) {
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (newTotal < 0) {
      throw new InventoryError('INVENTORY_INVARIANT_VIOLATED', 'total_quantity cannot be negative', 422);
    }
