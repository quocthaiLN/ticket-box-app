...
export async function validateTicketOffline(
  ...
  if (!ticket) {
    ...
    await insertQueue(db, clientItemId, 'TICKET', qrToken, null, concertId, gateId, now, 'INVALID_TICKET', 'Vé không hợp lệ!');
  ...
  if (!zoneAllowed) {
    await insertQueue(db, clientItemId, 'TICKET', qrToken, ticket.ticket_id, concertId, gateId, now, 'WRONG_GATE', 'Vé đi sai cổng!');
  ...
  if (isAlreadyCheckedIn) {
    await insertQueue(db, clientItemId, 'TICKET', qrToken, ticket.ticket_id, concertId, gateId, now, 'ALREADY_CHECKED_IN', 'Vé đã soát!');
...
async function insertQueue(
  ...
  await db.runAsync(`
    INSERT INTO offline_queue (client_item_id, type, qr_token, guest_id, phone, concert_id, gate_id, scanned_at, status, message)
    VALUES (?, ?, ?, NULL, NULL, ?, ?, ?, ?, ?)
  `, [clientItemId, type, qrToken, concertId, gateId, scannedAt, status, message]);
}
