...
export async function listConcerts(params: Record<string, string> = {}) {
  const response = await apiGet<ApiCollectionResponse<ConcertSummary>>(`/concerts${queryString(params)}`);
  return response.data;
}

export async function getConcert(concertId: string) {
  const response = await apiGet<ApiResponse<ConcertDetail>>(`/concerts/${concertId}`);
  return response.data;
}

export async function getConcertMetadata(concertId: string) {
  const response = await apiGet<ApiResponse<ConcertMetadata>>(`/concerts/${concertId}/metadata`);
  return response.data;
}

export async function listTicketTypes(concertId: string, includeClosed = false) {
...
export async function getInventory(concertId: string) {
  const response = await apiGet<ApiResponse<Inventory>>(`/concerts/${concertId}/inventory`);
  return response.data;
}

export async function listVenues() {
  const response = await apiGet<ApiCollectionResponse<Venue>>("/admin/venues?limit=100");
  return response.data;
}

export async function listAdminConcerts() {
  const response = await apiGet<ApiCollectionResponse<ConcertSummary>>("/admin/concerts?limit=100&sort=-starts_at");
  return response.data;
}
...
export function publishConcert(concertId: string) {
  return apiPost<ApiResponse<ConcertDetail>>(`/admin/concerts/${concertId}/publish`);
}

export function createSeatZone(concertId: string, input: Record<string, unknown>) {
  return apiPost<ApiResponse<SeatZone>>(`/admin/concerts/${concertId}/seat-zones`, input);
}

export function createTicketType(concertId: string, input: Record<string, unknown>) {
  return apiPost<ApiResponse<TicketType>>(`/admin/concerts/${concertId}/ticket-types`, input);
}

async function apiRequest<TData>(path: string, init: RequestInit): Promise<TData> {
