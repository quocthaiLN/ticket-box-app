...
const ARTIST_IMAGE_BUCKET =
  process.env.SUPABASE_ARTIST_IMAGE_BUCKET ?? "artist-images";
...
export async function uploadArtistImage(
  ...
  const { error } = await supabase()
    .storage.from(ARTIST_IMAGE_BUCKET)
