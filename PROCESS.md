...
Lưu ý: đây là trạng thái cleanup trước commit. Worktree hiện vẫn có thay đổi cần commit/chốt, nhưng các file không nên đưa lên remote đã được untrack/ignore đúng hướng.
...
### Quang: Check-in / Mobile / Guest List

Phạm vi chính: bảo đảm vé/guest QR check-in được cả online/offline, có xử lý duplicate và sync lại ổn định.
...
- Bổ sung test cho ticket `CANCELLED`/`REFUNDED`, device inactive/sai staff, hoặc guest import job nếu seed và service hiện tại hỗ trợ đủ.
- Chuẩn bị seed data cho checker account, gate, event, ticket, guest để demo không phụ thuộc thao tác thủ công.
...
### Thuận: Frontend / Demo / Documentation / DevOps

Phạm vi chính: giữ frontend production ở `ticket-box-app/apps/web`, làm demo rõ vai trò, và giữ repo sạch để nộp.
...
- Viết demo scripts 1 phút cho Audience, Organizer, Admin Web và Checker; mỗi script có mục tiêu, tài khoản, dữ liệu cần có, bước quay, kết quả mong đợi.
